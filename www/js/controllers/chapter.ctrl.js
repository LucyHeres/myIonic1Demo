  ...
  .controller('chapterCtrl', ['$scope', '$$strawberry', '$stateParams', '$state',
    function ($scope, $$strawberry, $stateParams, $state) {
      ...
      function chapterContent(chapter_index) {
        $$strawberry.getReadChapter({id: bookid, index: chapter_index}, {
          onSuccess: function (data) {
            if (!data.error) {
              console.log("获取到第" + chapter_index + "章节内容成功", data.result);
              $scope.chapterContent = data.result;
              //分段
              $scope.paragraphs=$scope.chapterContent.content.split(/\s+/);

            }else{
              console.log("获取到第" + chapter_index + "章节内容失败", data.error);
            }
          },
          onError: function () {
          }
